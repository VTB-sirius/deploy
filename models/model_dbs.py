...
class Main:
    ...
    def get_json(self, tf_idf, data, umap_data, labels, prj_id):
    ...
    def predict(self, prj_id):
        ...
        labels = dbs.labels_
        self.labels = labels
        plt_data = self.plot_data(emb_data, labels, PCA(n_components=2).fit_transform(emb_data))
        tf_idf = self.get_tfifd_v2_keywords(self.data, labels)

        answer = self.get_json(tf_idf, self.data, umap_data, labels, prj_id)

        return answer

    def get_topics_by_id(self, prj_id):
        arr = []
        try:
            for id_, text in enumerate(self.data):
                arr.append({
                    "_id": prj_id + '_dbs_' + str(id_),
                    "content":text,
                    "distribution": [{
...
def main_dbs():
    ...
    info = main.predict(prj_id=input_data['prj_id'])
    info = json.loads(json.dumps(info, cls=NpEncoder))
    result = main.get_topics_by_id(prj_id=input_data['prj_id'])
    ...
    newValue = { "$set": { 'dbs_payload': info['payload'] } }
